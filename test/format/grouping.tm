...
func wide(c:Bool -> Int)
    ...
    # table entry's `:` need it bounded just the same.
    >> [
        (if c then 111111111111111111 else 22222222222222222222 + 333333333333333333 + 44444444444444),
        3,
    ]
    >> {
        1: (if c then 111111111111111111 else 22222222222222222222 + 333333333333333333 + 444444444444),
    }
...
func word_prefix(optional:Int?, a:Bool, b:Bool)
    # `not` is a word, and a word in front of an expression reads as taking all
    # of it: `not x == y` is `not (x == y)` in most places it can be written,
    # and `(not x) == y` here. It binds tighter than every binary operator, so
    # nothing else was giving it parentheses.
    assert (not optional) == no
    assert a and (not b)
    # Standing alone it needs none:
    assert not optional
