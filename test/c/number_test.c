...
int main(void)
{
    // --- Basic integer arithmetic (integer sub-path) ---
    {
        ...
        number four = number_add(two, two);
    ...
    // caller. ---
    {
        ...
        for (size_t i = 0; i < n_ints; i++) {
            for (size_t j = 0; j < n_ints; j++) {
                number a1 = number_from_int(small_ints[i]), a2 = number_from_int(small_ints[i]);
                number b1 = number_from_int(small_ints[j]), b2 = number_from_int(small_ints[j]);
        ...
        for (size_t i = 0; i < n_frac_nums; i++) {
            for (size_t j = 0; j < sizeof(frac_dens) / sizeof(*frac_dens); j++) {
                ...
                number b1 = number_from_ratio(bn, bd), b2 = number_from_ratio(bn, bd);
                expect_dispatch_eq(number_add(a1, b1), number_add_general(a2, b2));
        ...
        {
            number big1 = number_from_string("123456789012345678901234567890");
            number big2 = number_from_string("123456789012345678901234567890");
            number small1 = number_from_int(7), small2 = number_from_int(7);
    ...
    // since neither is a `number` result). ---
    {
        ...
        for (size_t i = 0; i < n_cmp_ints; i++) {
            for (size_t j = 0; j < n_cmp_ints; j++) {
                number a1 = number_from_int(cmp_ints[i]), a2 = number_from_int(cmp_ints[i]);
                number b1 = number_from_int(cmp_ints[j]), b2 = number_from_int(cmp_ints[j]);
        ...
        for (size_t i = 0; i < n_cmp_frac_nums; i++) {
            for (size_t j = 0; j < sizeof(cmp_frac_dens) / sizeof(*cmp_frac_dens); j++) {
                ...
                number c1 = number_from_ratio(an, ad), c2 = number_from_ratio(an, ad);
                number d1 = number_from_ratio(an, ad), d2 = number_from_ratio(an, ad);
        ...
        // number_*_general's own same-pointer/same-tier logic untouched.
        {
            ...
            number shared = number_from_string("123456789012345678901234567890");
            number shared_ref = (shared);
        ...
        // logic.
        {
            number e1 = number_from_ratio(1, 0), e2 = number_from_ratio(1, 0); // div by zero -> error
            number seven1 = number_from_int(7), seven2 = number_from_int(7);
    ...
    {
        number third = number_from_ratio(1, 3);
        number sixth = number_from_ratio(1, 6);
        number sum = number_add(third, sixth);
    ...
    {
        number m = number_from_int(1 << 20);
        number big = number_mul(m, m); // 2^40: overflows the small form
        CHECK(tag_of(big) == 0);
        expect_str((big), 0, "1099511627776", 1);
        number back = number_div(big, m); // demotes to small again
        CHECK(tag_of(back) == 1);
        expect_eq(back, (m));

        number f30 = factorial(30);
        CHECK(tag_of(f30) == 0);
        expect_str((f30), 0, "265252859812191058636308480000000", 1);
        number f29 = factorial(29);
        number ratio = number_div(f30, f29);
    ...
    // --- Exact conversion from double ---
    {
        ...
        number tenth = number_from_double(0.1);
        bool exact;
        ...
        number reparsed = number_from_string(s);
    ...
    // --- Correctly rounded conversion to double ---
    {
        ...
        number x = number_from_string("1e300");
    ...
    // --- number_from_decimal: the exact-literal constructor ---
    {
        ...
        {
            number exact = number_from_decimal("3.15");
            number viadbl = number_from_double(3.15);
    ...
    {
        number third = number_from_ratio(1, 3);
        number approx = number_from_string("0.333");
        CHECK(number_compare(third, approx) > 0);
        CHECK(number_compare(approx, third) < 0);

        number a = number_from_int(-5), b = number_from_int(3);
        CHECK(number_compare(a, b) < 0);

        number f25 = factorial(25), f26 = factorial(26);
        ...
        number pi = number_pi();
        number r31 = number_from_string("3.1");
        ...
        number sin1 = number_sin(one), cos1 = number_cos(one);
        number sin1sq = number_mul(sin1, sin1), cos1sq = number_mul(cos1, cos1);
        number pyth = number_add(sin1sq, cos1sq); // sin(1)^2 + cos(1)^2, == 1
    ...
    // --- Unary operations ---
    {
        ...
        number x = number_from_ratio(-2, 3);
        ...
        number big = factorial(30);
        number negbig = number_neg(big);
        CHECK(number_sign(negbig) == -1);
        expect_eq(number_abs(negbig), (big));
        number invbig = number_inverse(big);
        number roundtrip = number_inverse(invbig);
    ...
    // --- Errors and propagation ---
    {
        ...
        number one = number_from_int(1), zero = number_from_int(0);
    ...
    // --- Error messages ---
    {
        ...
        {
            number pi2 = number_pi(), two = number_from_int(2);
            number half_pi = number_div(pi2, two);
        ...
        {
            number bad = number_div(number_from_int(1), number_from_int(0));
            number five = number_from_int(5);
        ...
        {
            number a = number_div(number_from_int(2), number_from_int(0));
    ...
    // --- Tier 3: sqrt ---
    {
        ...
        number prod = number_mul(s2, s2);
        CHECK(number_is_rational(prod));
        expect_eq(prod, (two));

        // sqrt(8) == 2*sqrt(2): commensurable radicands are recognized.
        number s8 = number_sqrt(number_from_int(8));
        number twice_s2 = number_mul(two, s2);
        CHECK(number_equal(s8, twice_s2));
        number q = number_div(s8, s2);
        expect_eq(q, (two));
        number three = number_from_int(3);
        number sum = number_add(s8, s2); // sqrt(8) + sqrt(2) == 3*sqrt(2)
        number three_s2 = number_mul(three, s2);
        CHECK(number_equal(sum, three_s2));

        // Field arithmetic in Q(sqrt 2): (1+sqrt2)^2 = 3+2sqrt2,
        // and 1/(1+sqrt2) = sqrt2 - 1.
        number one = number_from_int(1);
        number onePlus = number_add(one, s2);
        number sq = number_mul(onePlus, onePlus);
        number three2 = number_from_int(3);
        number two_s2 = number_mul(two, s2);
        number expected = number_add(three2, two_s2);
        CHECK(number_equal(sq, expected));
        number inv = number_inverse(onePlus);
        number s2m1 = number_sub(s2, one);
        CHECK(number_equal(inv, s2m1));

        // sqrt(3)/sqrt(27) == 1/3 and sqrt(2)*sqrt(3) == sqrt(6)
        number s3 = number_sqrt(number_from_int(3));
        number s27 = number_sqrt(number_from_int(27));
        number third = number_div(s3, s27);
        expect_eq(third, number_from_ratio(1, 3));
        number s6a = number_mul(s2, s3);
        number s6b = number_sqrt(number_from_int(6));
        CHECK(number_equal(s6a, s6b));

        // Incommensurable sums fall back to the general IRRATIONAL engine.
        number sum23 = number_add(s2, s3);
        ...
        number nested = number_sqrt(s2); // borrows s2
    ...
    {
        number z = number_from_int(0), o = number_from_int(1), negone = number_from_int(-1);
        ...
        number pi = number_pi();
        number tau = number_tau();
        CHECK(!number_is_rational(tau));
        number two_pi = number_mul(number_from_int(2), pi);
        CHECK(number_equal(tau, two_pi)); // tau == 2*pi, exactly
        expect_str((tau), 10, "6.2831853072", 0);

        number sq2a = number_sqrt2();
        number sq2b = number_sqrt(number_from_int(2));
        CHECK(number_equal(sq2a, sq2b));
        number prod = number_mul(sq2a, sq2a);
    ...
    // sinh/cosh/tanh/pow) ---
    {
        ...
        {
            number n2 = number_log10(number_from_int(2));
        ...
        // --- LN(r)/EXP(r) closed symbolic forms ---
        {
            ...
            number back = number_exp(ln3);
            expect_eq(back, (three));
            number half = number_from_ratio(1, 2);
            number exp_half = number_exp(half);
            CHECK(!number_is_rational(exp_half));
            number back2 = number_ln(exp_half);
            expect_eq(back2, (half));

            // exp(a)*exp(b) == exp(a+b): a closed EXP(5), not an
            // unrecognized product of two EXP nodes.
            number two = number_from_int(2);
            number e2 = number_exp(two);
            number e3 = number_exp(three);
            number prod = number_mul(e2, e3);
            expect_sym((prod), "exp(5)");
            number e5 = number_exp(number_from_int(5));
            expect_eq((prod), e5);

            // exp(a)/exp(b) == exp(a-b).
            number e2b = number_exp(two);
            number e3b = number_exp(three);
            number quot = number_div(e2b, e3b);
            number e_neg1 = number_exp(number_from_int(-1));
            expect_eq(quot, e_neg1);

            // ln(a) + ln(b) == ln(a*b): 2*ln(2) + ln(3) collapses to ln(12).
            number ln2 = number_ln(two);
            number two_ln2 = number_mul(two, ln2);
            number ln3b = number_ln(three);
            number sum = number_add(two_ln2, ln3b);
            expect_sym((sum), "ln(12)");
            number ln12 = number_ln(number_from_int(12));
            expect_eq(sum, ln12);

            // 2*ln(2) - ln(4) == 0 exactly: integer-coefficient LN terms
            // with different (but related) arguments still combine.
            number ln2b = number_ln(two);
            number two_ln2b = number_mul(two, ln2b);
            number ln4 = number_ln(number_from_int(4));
            expect_eq(number_sub(two_ln2b, ln4), number_from_int(0));

            // ln(1/2) == -ln(2): the < 1 canonicalization.
            number ln_half = number_ln(number_from_ratio(1, 2));
            number ln2c = number_ln(two);
            ...
            number ln3c = number_ln(three);
            number half_ln3 = number_mul(number_from_ratio(1, 2), ln3c);
            number ln2d = number_ln(two);
            number mixed = number_add(ln2d, half_ln3);
            CHECK(!number_is_rational(mixed));
            number e2c = number_exp(two);
        ...
        // correctly fall through to the general engine.
        {
            ...
            number pi = number_pi(), two = number_from_int(2), three = number_from_int(3);
            number half_pi = number_div(pi, two);
            number neg_half_pi = number_neg(half_pi);
            number two_pi = number_mul(two, pi);
            number three_half_pi = number_mul(three, half_pi);
        ...
        {
            number half = number_from_ratio(1, 2);
            number neg_half = number_neg(half);
            number two = number_from_int(2);
            number three = number_from_int(3);
            number four = number_from_int(4);
            number six = number_from_int(6);
            number sq2 = number_sqrt(two);
            number sqrt2_2 = number_div(sq2, two);
            number sq3 = number_sqrt(three);
            number sqrt3_2 = number_div(sq3, two);
            number neg_sqrt2_2 = number_neg(sqrt2_2);
            number neg_sqrt3_2 = number_neg(sqrt3_2);

            number pi = number_pi();
            number pi6 = number_div(pi, six);
            number pi4 = number_div(pi, four);
            number pi3 = number_div(pi, three);
        ...
        {
            number pi = number_pi();
            number pi12 = number_div(pi, number_from_int(12));
            number s1 = number_sin(pi12);
            CHECK(!number_is_rational(s1));
            expect_str(s1, 30, "0.258819045102520762348898837624", 0);
            number pi5 = number_div(pi, number_from_int(5));
            number s2 = number_sin(pi5);
            CHECK(!number_is_rational(s2));
        ...
        {
            number half = number_from_ratio(1, 2);
            number sq2 = number_sqrt(number_from_int(2));
            number sqrt2_2 = number_div(sq2, number_from_int(2));
            number sqrt3 = number_sqrt(number_from_int(3));
            number sqrt3_2 = number_div(sqrt3, number_from_int(2));
            ...
            number neg_sqrt3 = number_neg(sqrt3);
        ...
        number p1 = number_pow(number_from_int(2), number_from_int(10));
        CHECK(number_is_rational(p1));
        expect_str(p1, 5, "1024", 1);
        number p2 = number_pow(number_from_int(2), number_from_int(-3));
        CHECK(number_is_rational(p2));
        expect_str(p2, 5, "0.125", 1);
        number p3 = number_pow(number_from_int(-2), number_from_int(3));
        ...
        expect_str(p4, 5, "2", 1);
        number p5 = number_pow(number_from_int(27), number_from_ratio(2, 3));
        CHECK(number_is_rational(p5));
        expect_str(p5, 5, "9", 1);
        number p6 = number_pow(number_from_int(4), number_from_ratio(3, 2));
        CHECK(number_is_rational(p6));
        expect_str(p6, 5, "8", 1);
        number p7 = number_pow(number_from_int(-8), number_from_ratio(1, 3));
        CHECK(number_is_rational(p7));
        expect_str(p7, 5, "-2", 1);
        number p8 = number_pow(number_from_int(-8), number_from_ratio(2, 3));
        CHECK(number_is_rational(p8));
        expect_str(p8, 5, "4", 1);
        number p9 = number_pow(number_from_ratio(1, 8), number_from_ratio(1, 3));
        ...
        number p10 = number_pow(number_from_int(2), number_from_ratio(1, 3));
        ...
        number sqrt_pi = number_sqrt(pi); // borrows pi
        ...
        // deleted.
        {
            number x = number_from_int(2);
            number sinx = number_sin((x));
            number cosx = number_cos(x);
            number sin2 = number_mul(sinx, sinx);
            number cos2 = number_mul(cosx, cosx);
            number sum2 = number_add(sin2, cos2);
    ...
    // --- Symbolic form ---
    {
        ...
        number twothirds = number_from_ratio(2, 3);
    ...
    // --- Power collapsing: x*x*x displays as x^3 ---
    {
        ...
        number pi2 = number_mul(pi, pi); // bitwise-identical operands (pi is a singleton)
        expect_sym((pi2), "pi^2");
        expect_tex((pi2), "\\pi^{2}");
        ...
        number tan_a = number_tan(two);
        number tan_b = number_tan(two);
    ...
    // --- Infix operands that render looser than their context get parens ---
    {
        ...
        number pisum = number_add(pi, s2); // "pi + sqrt(2)": a SUM node
    ...
    // --- Rounding: floor/ceil/trunc/round across all tiers ---
    {
        ...
        for (size_t i = 0; i < sizeof(cases) / sizeof(*cases); i++) {
            number x = number_from_ratio(cases[i].n, cases[i].d);
        ...
        number f = factorial(25);
        number twentynine = number_from_int(29);
        number x = number_div(f, twentynine);
        number fl = number_floor(x), ce = number_ceil(x);
    ...
    // --- number_mod ---
    {
        ...
        for (size_t i = 0; i < sizeof(cases) / sizeof(*cases); i++) {
            ...
            number b = number_from_ratio(cases[i].bn, cases[i].bd);
            number m = number_mod(a, b);
    ...
    // here). ---
    {
        ...
        number e2 = number_exp(one);
        CHECK(number_compare(e2, pi) < 0);
        CHECK(number_compare(pi, e2) > 0);
        number lo_ref = number_from_ratio(2718281828, 1000000000);
        number hi_ref = number_from_ratio(2718281829, 1000000000);
    ...
    {
        number pi = number_pi();
        number halfpi = number_mul(pi, NUMBER_SMALL(1, 2));

        number up = number_atan2(number_from_int(1), number_from_int(0)); // +pi/2
        CHECK(number_equal(up, halfpi));

        number down = number_atan2(number_from_int(-1), number_from_int(0)); // -pi/2
        number neghalfpi = number_neg(halfpi);
        CHECK(number_equal(down, neghalfpi));

        number left = number_atan2(number_from_int(0), number_from_int(-1)); // pi
        CHECK(number_equal(left, pi));

        number origin_r = number_atan2(number_from_int(0), number_from_int(1)); // 0
        CHECK(number_is_zero(origin_r));

        number pi4 = number_mul(pi, NUMBER_SMALL(1, 4));
        number a11 = number_atan2(number_from_int(1), number_from_int(1)); // pi/4
        CHECK(number_equal(a11, pi4));

        number pi34 = number_mul(pi, NUMBER_SMALL(3, 4));
        number a1m1 = number_atan2(number_from_int(1), number_from_int(-1)); // 3*pi/4
    ...
    // --- gcd/lcm: integers, sign normalization, zeros, rationals, bigints ---
    {
        ...
        number p100 = number_pow(number_from_int(2), number_from_int(100));
        number p60 = number_pow(number_from_int(2), number_from_int(60));
        number g = number_gcd(p100, p60); // 2^60
        CHECK(number_equal(g, p60));
        number l = number_lcm(p100, p60); // 2^100
    ...
    // --- min/max: exact where decidable, `digits` fallback for ties ---
    {
        ...
        number pi = number_pi(), three = number_from_int(3);
        number mn = number_min(pi, three, 10); // pi > 3
        CHECK(number_equal(mn, three));
        number mx = number_max(pi, three, 10);
        CHECK(number_equal(mx, pi));

        number s2 = number_sqrt2(), f14 = number_from_ratio(7, 5); // sqrt(2) > 1.4
        number mx2 = number_max(s2, f14, 10);
