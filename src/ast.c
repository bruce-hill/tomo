...
const binop_info_t binop_info[NUM_AST_TAGS] = {
    ...
    [UnsignedLeftShift] = {"unsigned_left_shifted", "<<<"},
    [UnsignedLeftShiftUpdate] = {"unsigned_left_shifted", "<<<="},
    [UnsignedRightShift] = {"unsigned_right_shifted", ">>>"},
    [UnsignedRightShiftUpdate] = {"unsigned_right_shifted", ">>>="},
    [And] = {"bit_and", "and"},
...
#undef BINOP
        ...
        T(Test, "(Test ", quoted_text(data.label),
          data.expected_failure ? Texts(" fails ", quoted_text(data.expected_failure)) : EMPTY_TEXT,
          data.expected_compile_error ? Texts(" fails_compile ", quoted_text(data.expected_compile_error)) : EMPTY_TEXT,
          " ", ast_to_sexp(data.body), ")");
