// number.h — a general-purpose exact numeric datatype.
//
...
// rational) — identities like sqrt(2)*sqrt(2) == 2 and pi - pi == 0 hold
...
// Exact conversion — every finite double is a rational. NaN/Inf -> error.
...
// ratio form is rejected (it is not decimal notation -- use
...
// turn a source literal into a `number` with zero runtime cost -- no
// number_from_string call, no heap allocation, nothing but a 64-bit
// immediate load. NUMBER_SMALL(num, den) is a genuine C constant
// expression -- usable directly in a `static const number x = NUMBER_SMALL(...);`
// file-scope initializer, not just something an optimizer happens to fold
// at -O1+ -- that packs an ALREADY REDUCED rational straight into the
// immediate tier, the same representation small_make (number.c) builds at
// runtime, bypassing every runtime code path entirely.
//
...
// don't emit `number_from_double(3.15)`) -- 3.15 is not exactly
...
// (or number_from_string for a ratio) call for it instead -- both exact, no
// double round-trip (cache the result -- e.g. in a lazily-initialized
// static -- so a "very big or precise" literal still only pays that cost
// once, not on every use).
//
...
// false -- false means "not provably an integer", not "provably not".
...
// checked directly (disassembly of an LTO build still showed a real `call`
// for the full function). gnu_inline + always_inline is what lets a
// *narrow slice* of that logic -- just the part worth duplicating at every
// call site -- get inlined unconditionally, while the rest stays a single,
...
// arithmetic -- no field extraction, no repack -- following Daan Leijen,
...
//     one mask-and-compare per operand -- no separate tag and denominator
...
//   - Bit 63 of each word IS the numerator field's sign bit, so the
//     classic signed-overflow tests -- ((a^w) & (b^w)) < 0 for addition,
...
extern inline __attribute__((gnu_inline, always_inline)) number number_div(number a, number b) {
    if (__builtin_expect((a.bits & 3) == 1 && (b.bits & 3) == 1, 1)) {
        ...
        if (__builtin_expect(da == 1 && db == 1, 1)) {
            ...
            // general -- 3/4 is a genuine fraction needing real gcd
            ...
            // |a/b| <= |a| means it's automatically back in bounds -- no
            // separate overflow check needed, unlike the other three.
...
extern inline __attribute__((gnu_inline, always_inline)) number number_neg(number x) {
    if (__builtin_expect((x.bits & 3) == 1, 1)) {
        ...
        // comment above) -- no bounds check needed, unlike add/sub/mul.
...
// atan2(y, x): the angle in (-pi, pi] of the point (x, y) -- atan(y/x) placed
// in the correct quadrant by the signs of x and y. Exact on the axes and
// wherever number_atan is (atan2(1, 1) == pi/4, atan2(1, -1) == 3*pi/4).
...
extern inline __attribute__((gnu_inline, always_inline)) bool number_equal(number a, number b) {
    ...
    //     rational only ever proves nonzero or gives up -- both reported
    //     not-equal (see number_compare's contract above).
...
// terminating answer rather than propagating the "unordered" result. On a tie
// -- an exact equality, or indistinguishability at `digits` -- the first
// operand (a) is returned. An error operand propagates. (`digits` is ignored
...
// set to whether the conversion succeeded -- 0 with *ok true really is the
...
// product collapse into powers -- pi*pi*pi is "pi^3", not "pi*pi*pi"
