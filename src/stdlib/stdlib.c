...
static _Noreturn void signal_handler(int sig, siginfo_t *info, void *userdata) {
    ...
    // A crash skips atexit(), so run the cleanups here instead. Draining the
    // list makes this idempotent, which matters because the SIGILL branch
    // above raises SIGABRT and lands back in this handler.
    tomo_cleanup();
    raise(sig);
    _exit(1);
}
