...
ast_t *parse_infix_expr(parse_ctx_t *ctx, const char *pos, ast_e outer_op) {
    ...
    spaces(&pos);
    for (ast_e op; (op = match_binary_operator(&pos)) != Unknown && absorbs_rhs(outer_op, op_tightness[op]);
         spaces(&pos)) {
        ast_t *key = NULL;
        if (op == Min || op == Max) {
            ...
            for (bool progress = true; progress;) {
                ...
                progress =
                    (false || (new_term = parse_index_suffix(ctx, key))
                     ...
                     || (new_term = parse_fncall_suffix(ctx, key)) || (new_term = parse_record_literal_suffix(ctx, key))
